...
void Gpu::renderTile(int32_t idx, uint8_t startY, uint8_t startX, uint16_t mapStartAddr, uint16_t tileDataAddr)
{
    ...
    for (int x = 0; x < 8; ++x)
    {
        ...
        assert(((scanline_ * gameboy_width) + (x + (idx * 8))) < gameboy_width * gameboy_height);

        if (color == 0)
            pixels_[(scanline_ * gameboy_width) + (x + (idx * 8))] = SDL_MapRGB(window_surface_->format, 0xff, 0xff, 0xff);
        else if (color == 1)
            pixels_[(scanline_ * gameboy_width) + (x + (idx * 8))] = SDL_MapRGB(window_surface_->format, 0x44, 0x44, 0x44);
        else if (color == 2)
            pixels_[(scanline_ * gameboy_width) + (x + (idx * 8))] = SDL_MapRGB(window_surface_->format, 0xaa, 0xaa, 0xaa);
        else if (color == 3)
            pixels_[(scanline_ * gameboy_width) + (x + (idx * 8))] = SDL_MapRGB(window_surface_->format, 0, 0, 0);
